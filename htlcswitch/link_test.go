...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, cleanUp, err := createTwoClusterChannels(
		acmutil.SatoshiPerBitcoin*3,
		acmutil.SatoshiPerBitcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = acmutil.SatoshiPerBitcoin
...
func TestLinkForwardTimelockPolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 ACM over a 2-hop (3 vertex) route.
	amount := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 ACM over a 2-hop (3 vertex) route. Given the
	// current default fee of 1 SAT, if we just send a single ACM over in
	// an HTLC, it should be rejected.
	amountNoFee := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		acmutil.SatoshiPerBitcoin*5, acmutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
// bob<->alice channel has insufficient ACM capacity/bandwidth. In this test we
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// We'll attempt to send 4 ACM although the alice-to-bob channel only
	// has 3 ACM total capacity. As a result, this payment should be
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * acmutil.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func newSingleLinkTestHarness(chanAmt, chanReserve acmutil.Amount) (
...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	const chanAmt = acmutil.SatoshiPerBitcoin * 5
	...
	// Next, we'll create an HTLC worth 1 ACM, and send it into the link as
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = acmutil.SatoshiPerBitcoin * 5
		numHtlcs  = 4
		halfHtlcs = numHtlcs / 2
	)

	// We'll start by creating a new link with our chanAmt (5 ACM). We will
	...
	// Next, we'll create an HTLC worth 1 ACM that will be used as a dummy
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// channel reserve.
	const chanAmt = acmutil.SatoshiPerBitcoin * 5
	const chanReserve = acmutil.SatoshiPerBitcoin * 1
	...
	// Next, we'll create an HTLC worth 3 ACM, and send it into the link as
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * acmutil.SatoshiPerBitcoin)
	invoice, htlc, _, err := generatePayment(htlcAmt, htlcAmt, 5, mockBlob)
	if err != nil {
		t.Fatalf("unable to create payment: %v", err)
	}
	...
	const bobChanAmt = acmutil.SatoshiPerBitcoin * 1
	const bobChanReserve = acmutil.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = acmutil.SatoshiPerBitcoin * 3
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, acmutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
