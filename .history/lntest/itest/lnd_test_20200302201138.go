...
>>>>>>> 1467cd4dd382f9de7dc7ee89d779324ee9d2b4da:lntest/itest/lnd_test.go
)
...
func createPayReqs(ctx context.Context, node *lntest.HarnessNode,
	paymentAmt acmutil.Amount, numInvoices int) ([]string, [][]byte,
	[]*lnrpc.Invoice, error) {
=======
...
<<<<<<< HEAD:lnd_test.go
	assertAmountSent := func(amt acmutil.Amount) {
		// Both channels should also have properly accounted from the
		// amount that has been sent/received over the channel.
		listReq := &lnrpc.ListChannelsRequest{}
		ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
...
<<<<<<< HEAD:lnd_test.go
	...
	timeTravel := func(node *lntest.HarnessNode) (func() error,
		*lnrpc.ChannelPoint, int64, error) {
		...
		// We'll first open up a channel between them with a 0.5 ACM
		// value.
		ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
		chanPoint := openChannelAndAssert(
...
<<<<<<< HEAD:lnd_test.go
	// Next, we'll give Ainz exactly 2 utxos of 1 ACM each, with one of
	// them being p2wkh and the other being a n2wpkh address.
	ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
