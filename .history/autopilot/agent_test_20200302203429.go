...
func (m *mockConstraints) ChannelBudget(chans []Channel,
	balance acmutil.Amount) (acmutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() acmutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() acmutil.Amount {
	return 1e8
}
...
func (m *mockChanController) SpliceIn(chanPoint *wire.OutPoint,
	amt acmutil.Amount) (*Channel, error) {
	return nil, nil
}
...
func setup(t *testing.T, initialChans []Channel) (*testContext, func()) {
	...
	var availableFunds acmutil.Amount = 10 * acmutil.SatoshiPerBitcoin
	...
	testCfg := Config{
		...
		Heuristic:      heuristic,
		ChanController: chanController,
		WalletBalance: func() (acmutil.Amount, error) {
			ctx.Lock()
			defer ctx.Unlock()
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	newChan := Channel{
		ChanID:   randChanID(),
		Capacity: acmutil.SatoshiPerBitcoin,
	}
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * acmutil.SatoshiPerBitcoin,
		},
	)
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * acmutil.SatoshiPerBitcoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []Channel{
		...
		{
			ChanID:   randChanID(),
			Capacity: acmutil.SatoshiPerBitcoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	testCtx.Lock()
	testCtx.walletBalance += acmutil.SatoshiPerBitcoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * acmutil.SatoshiPerBitcoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != acmutil.SatoshiPerBitcoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					acmutil.SatoshiPerBitcoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * acmutil.SatoshiPerBitcoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, acmutil.SatoshiPerBitcoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget acmutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation acmutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation acmutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	var channelBudget acmutil.Amount = 5 * acmutil.SatoshiPerBitcoin
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * acmutil.Amount(numNewChannels)
	...
	channelBudget = acmutil.SatoshiPerBitcoin * 3 / 2
	...
	// our budget.
	channelBudget = acmutil.SatoshiPerBitcoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
