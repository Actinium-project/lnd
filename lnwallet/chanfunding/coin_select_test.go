...
func TestCoinSelect(t *testing.T) {
	...
	type testCase struct {
		...
		expectedInput  []acmutil.Amount
		expectedChange acmutil.Amount
		expectErr      bool
	}
	...
	testCases := []testCase{
			...
			coins: []Coin{
				{
					TxOut: wire.TxOut{
						...
						Value:    1 * acmutil.SatoshiPerBitcoin,
					},
				},
			},
			outputValue: 0.5 * acmutil.SatoshiPerBitcoin,

			// The one and only input will be selected.
			expectedInput: []acmutil.Amount{
				1 * acmutil.SatoshiPerBitcoin,
			},
			// Change will be what's left minus the fee.
			expectedChange: 0.5*acmutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true),
		},
		{
			...
			outputValue: 1 * acmutil.SatoshiPerBitcoin,
			expectErr:   true,
		},
		{
			...
			outputValue: 1*acmutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true) - dust,
		...
		},
		{
			...
			outputValue: 1*acmutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, true),
...
func TestCoinSelectSubtractFees(t *testing.T) {
	...
	const feeRate = chainfee.SatPerKWeight(100)
	const dustLimit = acmutil.Amount(1000)
	const dust = acmutil.Amount(100)

	type testCase struct {
		name       string
		spendValue acmutil.Amount
		coins      []Coin

		expectedInput      []acmutil.Amount
		expectedFundingAmt acmutil.Amount
		expectedChange     acmutil.Amount
		expectErr          bool
	}
	...
	testCases := []testCase{
		{
			...
			spendValue: 1 * acmutil.SatoshiPerBitcoin,

			// The one and only input will be selected.
			expectedInput: []acmutil.Amount{
				1 * acmutil.SatoshiPerBitcoin,
			},
			expectedFundingAmt: 1*acmutil.SatoshiPerBitcoin - fundingFee(feeRate, 1, false),
			expectedChange:     0,
		},
		{
		...
		},
		{
			...
			spendValue: 1*acmutil.SatoshiPerBitcoin - dust,
