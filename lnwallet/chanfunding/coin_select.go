...
type ErrInsufficientFunds struct {
	amountAvailable acmutil.Amount
	amountSelected  acmutil.Amount
}
...
// change+fees.
func selectInputs(amt acmutil.Amount, coins []Coin) (acmutil.Amount, []Coin, error) {
	satSelected := acmutil.Amount(0)
	for i, coin := range coins {
		satSelected += acmutil.Amount(coin.Value)
		if satSelected >= amt {
...
// function properly.
func CoinSelect(feeRate chainfee.SatPerKWeight, amt acmutil.Amount,
	coins []Coin) ([]Coin, acmutil.Amount, error) {
...
func CoinSelectSubtractFees(feeRate chainfee.SatPerKWeight, amt,
	dustLimit acmutil.Amount, coins []Coin) ([]Coin, acmutil.Amount,
