...
func TestSimpleAddSettleWorkflow(t *testing.T) {
	...
	// unittest. The channel will be funded evenly with Alice having 5 ACM,
	// and Bob having 5 ACM.
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
	...
	// At this point, Bob should have 6 ACM settled, with Alice still having
	// 4 ACM. Alice's channel should show 1 ACM sent and Bob's channel
	// should show 1 ACM received. They should also be at commitment height
	// two, with the revocation window extended by 1 (5).
	mSatTransferred := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(acmutil.NewTx(commitTx))
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (acmutil.Amount(500) + htlcTimeoutFee(
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...acmutil.Amount) (
		*LightningChannel, *LightningChannel, func()) {

		// Create a test channel funded evenly with Alice having 5 ACM,
		// and Bob having 5 ACM. Alice's dustlimit is 200 sat, while
		// Bob has 1300 sat.
		aliceChannel, bobChannel, cleanUp, err := CreateTestChannels()
		if err != nil {
			t.Fatalf("unable to create test channels: %v", err)
	...
	belowDust := acmutil.Amount(500) + htlcTimeoutFee(feePerKw)
	aboveDust := acmutil.Amount(1400) + htlcSuccessFee(feePerKw)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := acmutil.Amount(acmutil.SatoshiPerBitcoin*4) -
		calcStaticFee(1)
	...
	expectedBalance := acmutil.Amount(acmutil.SatoshiPerBitcoin * 5)
	if aliceChannel.channelState.LocalCommitment.LocalBalance.ToSatoshis() !=
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal acmutil.Amount) {
	...
	// satoshis.
	dustLimit := acmutil.Amount(10000)
	setDustLimit(dustLimit)

	// Both sides currently have over 1 ACM settled as part of their
	...
	aliceFee := acmutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	...
	bobFee := acmutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	...
	aliceBal := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	// First, we'll add 3 HTLCs of 1 ACM each to Alice's commitment.
	const numHTLCs = 3
	htlcAmt := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin)
	...
	// Alice now has an available balance of 2 ACM. We'll add a new HTLC of
	// value 2 ACM, which should make Alice's balance negative (since she
	// has to pay a commitment fee).
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * acmutil.SatoshiPerBitcoin)
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * acmutil.SatoshiPerBitcoin)
	const numHTLCs = 30
...
func TestDesyncHTLCs(t *testing.T) {
	...
	// First add one HTLC of value 4.1 ACM.
	htlcAmt := lnwire.NewMSatFromSatoshis(4.1 * acmutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	// Alice now has gotten all her original balance (5 ACM) back, however,
	...
	// available.
	//
	// We try adding an HTLC of value 1 ACM, which should fail because the
	// balance is unavailable.
	htlcAmt = lnwire.NewMSatFromSatoshis(1 * acmutil.SatoshiPerBitcoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	// We set the remote required MaxPendingAmount to 3 ACM. We will
	// attempt to overflow this value and see if it gives us the
	// ErrMaxPendingAmount error.
	maxPending := lnwire.NewMSatFromSatoshis(acmutil.SatoshiPerBitcoin * 3)
	...
	// First, we'll add 2 HTLCs of 1.5 ACM each to Alice's commitment.
	// This won't trigger Alice's ErrMaxPendingAmount error.
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshis(1.5 * acmutil.SatoshiPerBitcoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
	...
	// We finally add one more HTLC of 0.1 ACM to Alice's commitment. This
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance acmutil.Amount) {
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel, func()) {
		...
		// are loaded with 5 ACM each.
		aliceChannel, bobChannel, cleanUp, err := CreateTestChannels()
		if err != nil {
			t.Fatalf("unable to create test channels: %v", err)
		}

		// We set the remote required ChanReserve to 0.5 ACM. We will
		// attempt to cause Alice's balance to dip below this amount
		// and test whether it triggers the ErrBelowChanReserve error.
		aliceMinReserve := acmutil.Amount(0.5 *
			acmutil.SatoshiPerBitcoin)
		...
		bobMinReserve := acmutil.Amount(6 * acmutil.SatoshiPerBitcoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * acmutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		...
		acmutil.SatoshiPerBitcoin*4.5-commitFee, acmutil.SatoshiPerBitcoin*5,
	)
	...
	// Now we'll add HTLC of 3.5 ACM to Alice's commitment, this should put
	// Alice's balance at 1.5 ACM.
	//
	// Resulting balances:
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshis(3.5 * acmutil.SatoshiPerBitcoin)
	...
	// Add a second HTLC of 1 ACM. This should fail because it will take
	...
	// Add a HTLC of 2 ACM to Alice, and the settle it.
	// Resulting balances:
	//	Alice:	3.0
	//	Bob:	7.0
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * acmutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		acmutil.SatoshiPerBitcoin*3-commitFee, acmutil.SatoshiPerBitcoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		acmutil.SatoshiPerBitcoin*3-commitFee, acmutil.SatoshiPerBitcoin*7,
	)

	// And now let Bob add an HTLC of 1 ACM. This will take Bob's balance
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		acmutil.SatoshiPerBitcoin*3-commitFee, acmutil.SatoshiPerBitcoin*6,
	)
}
...
func TestMinHTLC(t *testing.T) {
	...
	// We set Alice's MinHTLC to 0.1 ACM. We will attempt to send an
	// HTLC BELOW this value to trigger the ErrBelowMinHTLC error.
	minValue := lnwire.NewMSatFromSatoshis(0.1 * acmutil.SatoshiPerBitcoin)
	...
	// First, we will add an HTLC of 0.5 ACM. This will not trigger
	// ErrBelowMinHTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * acmutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
